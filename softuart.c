...
{
//    uint8_t data = uart->rx_queue_buffer[uart->ringbuffer_received_read_idx];
//    uart->ringbuffer_received_read_idx++;
...
SOFTWARE_UART_SECTION_IRAM void softuart_timer_rx_process_callback(softuart_t *uart)
{
	...
	{
		if (uart->read_rx_pin(uart->rx_pin))
		{
	...
	/* stop bit */
	if ((uart->read_rx_pin(uart->rx_pin) && (uart->rx_shift_position == 9)))
	{
